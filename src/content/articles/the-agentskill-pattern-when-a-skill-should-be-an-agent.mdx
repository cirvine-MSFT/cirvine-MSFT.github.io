...
**TL;DR** — Take a task you'd normally write as a skill, implement it as a *separate agent* with a cheaper model and a narrow tool set, and let your main session delegate to it as a sub-agent. I've been calling this the **AgentSkill pattern** because I couldn't find an official name for it. The honest scorecard: you almost always get **isolated context**. You *sometimes* get cheaper runs and faster execution — and to know which, you have to compare **combined credits**, not token counts, because a cheaper worker can burn more tokens and still cost less. Two things nobody puts on the whiteboard: quality can quietly get worse, and if the parent doesn't invoke your agent *by name*, your cheap model never runs at all. This post has the actual frontmatter for both Copilot and Claude Code, plus the gotchas that bit my team.
...
This is the mistake I keep catching myself making. You do all the work to isolate the specialist… and then the parent re-reads the artifact to "check it," re-runs it, tidies it up, or writes its own summary of the summary.
...
What the parent does *after* delegation is a design decision, not an afterthought. Decide deliberately whether it's allowed to touch the artifact at all — and if you find it always needs to, that's a strong signal this task wanted to stay inline.
...
Here's the thing that took me too long to internalize: **a skill is advisory prose aimed at the parent.** It is not a machine-enforced binding between a skill and an agent. Your routing skill says "delegate this to `pipeline-rca`" and then the parent, a language model, decides what to actually do with that suggestion. It might:
...
What would actually fix it is a first-class operation that binds a skill to exactly one agent, one model, and one tool allowlist — deterministically, without asking a language model to please remember. Until then, this is prose-directed selection, and prose-directed selection is probabilistic. Build your expectations accordingly.
